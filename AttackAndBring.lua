...
local function FixNetwork(v)
    pcall(function()
        if v:IsA("BasePart") then
            sethiddenproperty(v, "NetworkOwnershipRule", Enum.NetworkOwnershipRule.Manual)
...
BringEnemy = function()
    ...
    for _, v in pairs(workspace.Enemies:GetChildren()) do
        ...
        and v.Humanoid.Health > 0 then
            if (v.HumanoidRootPart.Position - PosMon).Magnitude <= getgenv().BringMode then
